...
import {BadRequestError, ForbiddenError, InternalServerError, NotFoundError, UnauthorizedError,} from '@packages-be/errors'
...
describe('Axios Interceptors Test Suite', () => {
  ...
  it('handle happy case where by response adheres to standard response format', () => {
    ...
    // Assert
    expect(interceptedResponse.originalUrl).toEqual(
      mockSuccessResponse.request.url
    )
    expect(interceptedResponse.status).toEqual(mockSuccessResponse.status)
    expect(interceptedResponse.statusText).toEqual(
      mockSuccessResponse.statusText
    )
  ...
  it('should intercept bad request error and wrap and re-throw as @packages-be/errors/BadRequest', async () => {
    // Assemble
    const onRejected = wrapAxiosErrorsInterceptor.onRejected
  ...
  it('should intercept Not Found error and wrap and re-throw as @packages-be/errors/NotFoundError', async () => {
    // Assemble
    const onRejected = wrapAxiosErrorsInterceptor.onRejected
  ...
  it('should intercept Unauthorised error and wrap and re-throw as @packages-be/errors/UnauthorisedError', async () => {
    // Assemble
    const onRejected = wrapAxiosErrorsInterceptor.onRejected
  ...
  it('should intercept InternalServer error and wrap and re-throw as @packages-be/errors/InternalServerError', async() => {
    // Assemble
    const onRejected = wrapAxiosErrorsInterceptor.onRejected
  ...
  it('should intercept Forbidden error and wrap and re-throw as @packages-be/errors/ForbiddenError', async () => {
    // Assemble
    const onRejected = wrapAxiosErrorsInterceptor.onRejected
