...
import {
  ...
  RequestContext,
} from "@packages-be/requestContext";
...
const mergeWithHeadersFromRequestContext = (
  ...
  if (
    requestContextProvider &&
    headerNamesToForwardFromRequestContext &&
    headerNamesToForwardFromRequestContext.length > 0
  ) {
    ...
    if (requestContext) {
      ...
      if (headersFromContext) {
        const headerFromRequestContext: HttpClientRequestHeaders =
          headerNamesToForwardFromRequestContext.reduce(
            (currentHeaders: HttpClientRequestHeaders, headerName: string) => {
              ...
              const headerValue =
                headersFromContext[headerName.toLowerCase()] ??
                headersFromContext[headerName];
              if (headerValue) {
